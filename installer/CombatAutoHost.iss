#ifndef AppName
  #define AppName "Multiplayer Combat Host"
#endif
...
[Setup]
AppId={{2D4D3B16-6D86-49D5-9DD7-43455C6C1423}
AppName={#AppName}
...
begin
  ...
  Candidate := PathJoin(ExpandConstant('{commonpf32}'), 'Steam');
  if DirExists(Candidate) then
  begin
    Result := NormalizeDir(Candidate);
    Exit;
  ...
  Candidate := PathJoin(ExpandConstant('{commonpf}'), 'Steam');
  if DirExists(Candidate) then
  begin
    Result := NormalizeDir(Candidate);
    Exit;
  ...
  Candidate := PathJoin(ExpandConstant('{localappdata}'), 'Programs\Steam');
  if DirExists(Candidate) then
  begin
    Result := NormalizeDir(Candidate);
    Exit;
  ...
  begin
    ...
    Candidate := Chr(DriveCode) + ':\SteamLibrary\Steam';
    if DirExists(Candidate) then
    begin
      Result := NormalizeDir(Candidate);
      Exit;
...
begin
  ...
  if IsSts2GameDir(Result) then Exit;
...
begin
  Result := True;
  if Assigned(GameDirPage) and (CurPageID = GameDirPage.ID) then
  begin
    ResolvedGameDir := '';
